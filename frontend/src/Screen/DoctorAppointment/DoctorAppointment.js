...
class DoctorAppointment extends Component {
  ...
  handleSubmit = (e) => {
    ...
    if (this.state.gender != "" && this.state.physician != "" && this.state.appointmentDate != "" && this.state.appointmentTime != "" && this.state.appointmentNote != "") {
      ...
      //---------------------
      // add appointment
      axios.post(`/codebusters/api/patientpvt/appointment/addappointments/${this.state.userId}`, appointment, config).then(res => {
        if (res.data.success) {
                ...
                <Row>
                  <Col>
                    <ButtonGroup aria-label="Basic example">
                      <Button variant="primary" style={{ marginTop: "20px" }} onClick={this.handlePayment}>
                        Request Appointment
                      </Button>
                      <Col>
